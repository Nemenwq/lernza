...
export function Dashboard({ onSelectWorkspace, onCreateQuest }: DashboardProps) {
  ...
  if (!connected) {
    return (
      <div className="min-h-[calc(100vh-67px)] flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute bottom-[15%] right-[6%] w-14 h-14 bg-primary border-2 border-border shadow-[3px_3px_0_var(--color-border)] -rotate-6 opacity-[0.1] animate-float" style={{ animationDuration: "6s", animationDelay: "1s" }} />
        <div className="absolute top-[60%] left-[5%] w-10 h-10 bg-success border-2 border-border shadow-[2px_2px_0_var(--color-border)] rotate-45 opacity-[0.06] animate-float" style={{ animationDuration: "7s", animationDelay: "2s" }} />
        <div className="absolute top-[20%] right-[12%] w-8 h-8 bg-primary border-2 border-border opacity-[0.07] -rotate-12 animate-float" style={{ animationDuration: "9s", animationDelay: "0.5s" }} />
  ...
  return (
    <div className="relative mx-auto max-w-6xl px-4 sm:px-6 py-8">
      {/* Welcome banner — bg-primary, so border-border gives yellow-on-yellow in dark;
          that's intentional: the banner itself is yellow and the border acts as a bold frame */}
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-5 relative">
        ...
        <div className="flex gap-0 border-2 border-border shadow-[3px_3px_0_var(--color-border)]">
          {(["all", "owned", "enrolled"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-2 text-xs font-black capitalize tracking-wider transition-colors cursor-pointer border-r-2 border-border last:border-r-0 ${
            ...
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <div className="w-8 h-8 bg-secondary border-2 border-border flex items-center justify-center shrink-0 ml-3 group-hover:bg-primary group-hover:shadow-[2px_2px_0_var(--color-border)] transition-all">
